...
@open_file(1, mode="wb")
def write_edgelist_undi(G, path, delimiter=" ", data=True, encoding="utf-8"):
...
@open_file(1, mode="wb")
def write_edgelist_di(G, path, delimiter=" ", data=True, encoding="utf-8"):
    for line in nx.readwrite.edgelist.generate_edgelist(G, delimiter, data):
        line += "\n"
        path.write(line.encode(encoding))
    return
...
if __name__ == "__main__":
    ...
    #random ER bipartite
    num_nodes_left_er = 6
    num_nodes_right_er = 6
    p_ER_bipartite_bi = 0.3 # p is the probability that an edge exists,
    q_ER_bipartite_bi = 0.3 # q is the probability that an edge is demanded
    #random HH bipartite
    num_nodes_HH = 30
    max_degree_HH = 8
    q_HH = 0.2
    ...
    m_ba=7
    #random network
    # multi unicast, p is the density of interference
    num_t, num_antenna, p_network = 25, 1, 0.1
    num_r = num_t
    #random PA_graph
    num_nodes_pre = 30
    max_degree_pre = 6
    ...
    if graph_type == 'random_bigraph':
        base_data_dir = os.path.join('../data/new_data_di_bipar_{}{}{}{}_{}_1000test'
                                 .format(num_nodes_left_er, num_nodes_right_er, p_ER_bipartite_bi, q_ER_bipartite_bi, num_graph))
    elif graph_type == 'er_graph':
        base_data_dir = os.path.join('../data/new_data_di_ER_{}_{}_{}_test_chromatic'
                                     .format( num_nodes_er, p_er, num_graph))
        ...
        base_data_dir = os.path.join('../data/new_data_di_network_{}{}{}{}_{}_test'
    ...
    for g_idx in tqdm(range(num_graph)):
        if graph_type == 'geo_graph':
            ...
            G_di = nx.fast_gnp_random_graph(n=num_nodes_er, p=p_er, directed=True)
            G_undi = nx.Graph(G_di)

        elif graph_type == 'ba_graph':
            G_undi = nx.generators.random_graphs.barabasi_albert_graph(n = num_nodes_ba, m = m_ba)
        elif graph_type == 'random_bigraph':
            G = generate_random_bigraph(num_nodes_left_er, num_nodes_right_er, p_ER_bipartite_bi, q_ER_bipartite_bi)
        ...
        write_edgelist_undi(G_undi, path = nx_g_path_undi, data=False)
        write_edgelist_di(G_di, path=nx_g_path_di, data=False)
