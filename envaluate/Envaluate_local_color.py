...
from envs import env_local_color
...
def envaluate_RL_local_same_model(data_dir,num_color,max_num_nodes, device, training_device, base_model_save_dir, save_graph
        ...
        for g_di in tqdm(data_loaders[mode]):  # input digraphs
            ...
            while True:
                ...
                if torch.all(done).item():
                    ...
                    succ_local_colored = np.apply_along_axis(lambda row: np.where(row <= num_color - 2, 1, 0), axis=1,
    ...
    if save_graph_or_not:
        suc_dir = os.path.join(data_dir, "local_color", "success")
        unsuc_dir = os.path.join(data_dir, "local_color", "unsuccess")
